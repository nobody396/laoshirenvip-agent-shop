    ...
    <div class="mx-auto max-w-3xl">
      ...
      <form v-else class="mt-8 space-y-6 rounded-2xl border bg-card p-6 shadow-sm" @submit.prevent="submit">
        <label class="grid gap-2 text-sm">
		  <span>{{ isRecharge ? '充值单号' : '订单号（选填，线下转账可不填）' }}</span>
          <Input v-model="form.order_no" @input="clearPreview" @blur="loadPreview" />
        </label>
		<div v-if="form.order_no && isGMShop" class="grid gap-2 text-sm"><span>下单邮箱</span><Input v-model="guest.email" type="email" required @input="clearPreview" @blur="loadPreview" /></div>
		<div v-else-if="form.order_no && isGuest" class="grid gap-4 md:grid-cols-2">
...
const supportsWalletPayment = computed(() => userAuthStore.isAuthenticated && !isGuest.value && !isGMShop.value)
...
async function loadPreview() {
  ...
  if (!orderNo && !String(form.invoice_amount || '').trim()) {
    preview.value = null
    previewError.value = ''
    return
  }
  ...
  try {
    ...
    if (!orderNo) response = userAuthStore.isAuthenticated ? await invoiceAPI.previewManual(payload) : await invoiceAPI.previewManualGuest(payload)
    else if (isGMShop.value) response = await invoiceAPI.previewGMShop({ ...payload, order_email: guest.email })
...
async function submit() {
  ...
  try {
	...
	if (!form.order_no.trim()) response = userAuthStore.isAuthenticated ? await invoiceAPI.createManual(payload) : await invoiceAPI.createManualGuest(payload)
	else if (isGMShop.value) response = await invoiceAPI.createGMShop({ ...payload, order_email: guest.email })
