...
func ProcessReadyInvoices(ctx context.Context, store DeliveryStore, source InvoiceDocumentSource, mailer InvoiceMailer) error {
	...
	for _, item := range items {
		...
		mailSource := request.Source
		if request.Source == "gmshop" || request.SourceHost == "laoshirenvip.com" {
			mailSource = "gmshop"
		...
		if sendErr := mailer.SendInvoice(mailSource, request.RecipientEmail, subject, body, fileName, content); sendErr != nil {
