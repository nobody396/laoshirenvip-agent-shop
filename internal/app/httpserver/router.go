...
func SetupRouter(cfg *config.Config, c *container.Container) *gin.Engine {
	...
	publicResellerTLSHandler := resellerHandlers.PublicTLS
	...
	userResellerCustomerWalletHandler := resellerHandlers.UserCustomerWallet
	userResellerCustomerPriceHandler := resellerHandlers.UserCustomerPrice
	...
	registerStorefrontRoutes(apiV1, cfg, c, publicContentHandler, publicCatalogHandler, publicCategoryHandler, publicResellerTLSHandler, userResellerHandler, userResellerProductSettingHandler, userResellerFinanceHandler, userResellerOrderHandler, userResellerCustomerWalletHandler, userResellerCustomerPriceHandler, userApiCredentialHandler, userAuditLogHandler, userGiftCardHandler, publicMemberLevelHandler, userProfileHandler, userEmailHandler, userPasswordHandler, userVerifyHandler, userTelegramOIDCHandler, userTelegramHandler, userGoogleHandler, userLoginHandler, user2FAHandler, publicConfigHandler, userCartHandler, userOrderHandler, guestOrderHandler, orderPreviewHandler, orderCreateHandler, paymentLatestHandler, paymentWriteHandler, userWalletHandler, redisClient, loginRule, guestReadRule, guestWriteRule)
	...
	invoicehttp.RegisterCallbackRoutes(apiV1, invoicehttp.NewHandler(c.InvoiceService, c.OrderService, c.GMShopInvoiceReader, c.WalletService, c.PaymentStore))
