...
func (c *Consumer) Register(mux *asynq.ServeMux) {
	...
	mux.HandleFunc(queue.TaskResellerProvisionDomains, withPanicRecovery(queue.TaskResellerProvisionDomains, c.handleResellerProvisionDomains))
	...
	mux.HandleFunc(queue.TaskInvoiceDelivery, withPanicRecovery(queue.TaskInvoiceDelivery, c.handleInvoiceDelivery))
}
